...
if [ "$options" -eq 1 ]; then
    ...
    docker exec -t "$CONTAINER_NAME" psql -U $database_name -f "/var/scripts/fix_indexes_$database_name.sql" -o "/var/scripts/$database_name-alter_indexes.sql"
    docker exec -t "$CONTAINER_NAME" psql -U $database_name -f "/var/scripts/alter_indexes_$database_name.sql" -o "/var/scripts/$database_name-alter_indexes.log"
    
    echo "-------------------------------------------------------"
    cat "/var/lib/pgsql/scripts/$database_name-alter_indexes.log"
    echo "-------------------------------------------------------"
else
